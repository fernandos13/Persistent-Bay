...
obj/machinery/recharger/wallcharger/attackby(obj/item/weapon/G as obj, mob/user as mob)
	if(default_deconstruction_screwdriver(user, G))
		updateUsrDialog()
		return 1
	if(default_deconstruction_crowbar(user, G))
...
/obj/machinery/recharger/wallcharger/attack_hand(mob/user as mob)
	if(istype(user,/mob/living/silicon))
		return
...
/obj/machinery/recharger/wallcharger/power_change()
	. = ..()
	if(powered())
		update_use_power(POWER_USE_IDLE)
	else
...
/obj/machinery/recharger/wallcharger/set_anchored(new_anchored)
	. = ..()
	if(anchored && operable())
		update_use_power(POWER_USE_IDLE)
	else
...
/obj/machinery/recharger/wallcharger/emp_act(severity)
	if(inoperable() || !anchored)
		..(severity)
		return
	var/obj/item/weapon/cell/C = get_charging_cell()
...
//Utiliy proc until we got a unified power supply datum or something
/obj/machinery/recharger/wallcharger/proc/get_charging_cell()
	if(!charging)
		return
	. = charging.get_cell()
	// if(istype(charging, /obj/item/device/suit_sensor_jammer))
...
/obj/machinery/recharger/wallcharger/on_update_icon()	//we have an update_icon() in addition to the stuff in process to make it feel a tiny bit snappier.
	if(charging)
		var/obj/item/weapon/cell/C = get_charging_cell()
		if(C && C.fully_charged())
			icon_state = icon_state_charged
...
obj/machinery/recharger/wallcharger/examine(mob/user)
	. = ..()
	if(!. || isnull(charging))
		return
